...
//! `mez-core::digest`. This test greps the workspace source for `sha2::Sha256`
//! usage and verifies it appears only in the three allowed locations:
//!
//! 1. `mez-core/src/canonical.rs` — canonicalization pipeline
//! 2. `mez-core/src/digest.rs` — digest module
//! 3. `mez-crypto/src/mmr.rs` — MMR node hashing (documented exception)
//!
...
/// The workspace root for the mez crates.
fn workspace_root() -> PathBuf {
    let manifest_dir = std::env::var("CARGO_MANIFEST_DIR").unwrap_or_else(|_| ".".to_string());
    PathBuf::from(manifest_dir)
        .parent() // up from mez-integration-tests
...
const ALLOWED_SHA256_FILES: &[&str] = &[
    "mez-core/src/canonical.rs",
    "mez-core/src/digest.rs",
    "mez-crypto/src/mmr.rs",
];

/// Verify that `sha2::Sha256` does not appear in any Cargo.toml
/// other than `mez-core` and `mez-crypto`.
#[test]
fn sha2_dependency_only_in_core_and_crypto() {
    ...
    for entry in entries {
        ...
        // Only mez-core and mez-crypto may have sha2 as a direct dependency.
        // mez-crypto needs raw sha2 for MMR node hashing (documented exception
        // per CLAUDE.md §V.5 — needs raw [u8; 32], not hex).
        if crate_name != "mez-core" && crate_name != "mez-crypto" {
            ...
            assert!(
                ...
                "VIOLATION: {crate_name}/Cargo.toml has a direct sha2 dependency. \
                 Per CLAUDE.md §V.5, only mez-core and mez-crypto (MMR exception) \
                 may depend on sha2 directly. \
                 Use mez_core::sha256_digest() or mez_core::sha256_raw() instead."
            );
        }
    }
}
        ...
        if !is_allowed && !is_test {
            panic!(
                "VIOLATION (B-005): sha2:: usage outside allowed files:\n{line}\n\n\
                 ...
                 Per CLAUDE.md §V.5, use mez_core::sha256_digest() instead."
            );
        }
    }
}
