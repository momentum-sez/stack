...
//! the same code paths that the CLI would invoke for `mez sign` and
//! `mez verify` subcommands.
...
fn make_vc() -> VerifiableCredential {
    VerifiableCredential {
        ...
        id: Some("urn:mez:vc:cli-test:001".to_string()),
...
#[test]
fn sign_vc_with_mez_proof_type() {
    let sk = SigningKey::generate(&mut OsRng);
    let mut vc = make_vc();
