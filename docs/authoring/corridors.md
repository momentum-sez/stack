...
```bash
python tools/msez.py corridor verify modules/corridors/<name>
```

You can sign a VC with:

```bash
python tools/msez.py vc sign path/to/unsigned.vc.json --key path/to/private.jwk --out corridor.vc.json
```
...
python tools/msez.py vc keygen --out keys/my-dev.ed25519.jwk

# Scaffold a corridor definition VC from corridor.yaml + artifact hashes
python tools/msez.py corridor vc-init-definition <corridor-dir> \
  --issuer did:key:z... \
  --out corridor.vc.unsigned.json

# Scaffold a participant-specific corridor agreement VC (party defaults issuer)
python tools/msez.py corridor vc-init-agreement <corridor-dir> \
  --party did:key:z... \
  --role zone_authority \
...
python tools/msez.py vc sign agreement.zone-a.unsigned.json \
  --key docs/examples/keys/zone-a.ed25519.jwk \
  --out corridor.agreement.zone-a.vc.json

# Zone B signs its own agreement VC
python tools/msez.py vc sign agreement.zone-b.unsigned.json \
...
```bash
python tools/msez.py corridor status <corridor-dir>
```
...
```bash
# signer 1
python tools/msez.py vc sign docs/examples/vc/unsigned.corridor-agreement.json \
  --key docs/examples/keys/dev.ed25519.jwk \
  --out corridor.agreement.vc.json

# signer 2 appends a second proof to the same file
python tools/msez.py vc sign corridor.agreement.vc.json \
...
```bash
python tools/msez.py vc payload-hash path/to/corridor.vc.json
```

Then verify that the agreement is valid **and activation thresholds are met**:

```bash
python tools/msez.py corridor verify <corridor-dir>
```
