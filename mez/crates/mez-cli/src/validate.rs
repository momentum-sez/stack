...
//!
//! Matches the behavior of `tools/mez.py validate --all-modules`, `--all-profiles`,
...
/// Arguments for the `mez validate` subcommand.
#[derive(Args, Debug)]
...
pub fn run_validate(args: &ValidateArgs, repo_root: &Path) -> Result<u8> {
    ...
    if !args.all_modules && !args.all_profiles && !args.all_zones && args.path.is_none() {
        println!("Usage: mez validate [--all-modules] [--all-profiles] [--all-zones] [PATH]");
        return Ok(1);
    }
...
mod tests {
    ...
    fn find_yaml_files_returns_empty_for_nonexistent_dir() {
        let files = find_yaml_files(
            Path::new("/tmp/mez-test-nonexistent-dir-xyz"),
            "module.yaml",
        );
    ...
    fn validate_all_modules_missing_dir_returns_false() {
        ...
        let validator = mez_schema::SchemaValidator::new(&schema_dir).unwrap();
    ...
    fn validate_single_path_nonexistent_returns_true() {
        ...
        let validator = mez_schema::SchemaValidator::new(&schema_dir).unwrap();
        let result =
            validate_single_path(&validator, Path::new("/tmp/mez-no-such-file.yaml")).unwrap();
