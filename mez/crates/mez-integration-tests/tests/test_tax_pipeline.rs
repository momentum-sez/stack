...
//! ```
//!
//! These tests exercise the real `mez-agentic::tax` module across all three
...
//!   composition, which is currently done in `mez-api` routes, not in a composable
//!   library function — a composable `process_fiscal_trigger()` function in
//!   `mez-agentic` would close this gap)
...
fn tax_and_agentic_taxonomies_are_independently_complete() {
    use mez_agentic::policy::TriggerType;
...
fn policy_engine_fiscal_trigger_deterministic() {
    ...
    use mez_agentic::policy::{Trigger, TriggerType};
