...
use mez_core::{sha256_bytes, sha256_digest, CanonicalBytes, ComplianceDomain, ContentDigest, MezError};
...
impl TensorCommitment {
    ...
    pub fn compute<J: JurisdictionConfig>(tensor: &ComplianceTensor<J>) -> Result<Self, MezError> {
    ...
    /// an empty cell map.
    pub fn empty(jurisdiction_id: &str) -> Result<Self, MezError> {
        let payload = CommitmentPayload {
...
/// by the MMR accumulator in `mez-crypto`.
///
...
mod tests {
    ...
    fn merkle_root_is_byte_level_not_string_level() {
        ...
        let expected_bytes = mez_core::sha256_bytes(&combined);
