...
module.exports = function build_chapter47() {
  return [
    ...
    p("The validation framework enforces structural and semantic correctness at every system boundary. All external inputs are validated against JSON Schemas (116 schemas in mez-schema) before processing. Internal types use newtype wrappers with validation on construction, ensuring that invalid states are unrepresentable."),
    table(
      ["Boundary", "Validation Method", "Error Handling", "Coverage"],
      [
        ["API request body", "JSON Schema (mez-schema, 116 schemas)", "400 with field-level diagnostic", "All POST/PUT endpoints"],
        ["Path/query params", "Newtype parsing with FromStr", "400 with expected format", "All parameterized routes"],
        ["Mass API responses", "Typed deserialization (serde)", "502 with upstream error context", "All mez-mass-client calls"],
    ...
    table(
      ...
      [
        ["Key zeroization", "Zeroize + ZeroizeOnDrop on Ed25519 SigningKey", "mez-crypto", "Key material persistence in memory after use"],
        ["Constant-time comparison", "subtle::ConstantTimeEq for all secret comparisons", "mez-api auth", "Timing side-channel on bearer token validation"],
        ["Canonical hashing", "CanonicalBytes::new() → SHA-256 for all digests", "mez-core", "Non-deterministic serialization producing different hashes"],
        ["Secret redaction", "SecretString with [REDACTED] Debug/Display", "mez-api", "Secret leakage in log output"],
        ["Auth token lifecycle", "Zeroizing<String> from env read to process shutdown", "mez-mass-client", "Token persistence in memory after config load"],
        ["MMR integrity", "Append-only Merkle Mountain Range for receipt chains", "mez-crypto", "Receipt chain tampering or reordering"],
      ],
      [2000, 2800, 1600, 2960]
    ),
