...
//! Validates against the JSON schema from `mez-schema` at construction time
...
pub const REGISTRY_SCHEMA_ID: &str =
    "https://schemas.momentum-ez.org/mez/vc.smart-asset-registry.schema.json";
...
impl SmartAssetRegistryVc {
    ...
    pub fn new(
        ...
        let vc = VerifiableCredential {
            ...
            id: Some(format!("urn:mez:vc:smart-asset-registry:{asset_id}")),
            credential_type: CredentialTypeValue::Array(vec![
                "VerifiableCredential".to_string(),
                "MezSmartAssetRegistryCredential".to_string(),
            ]),
            issuer,
    ...
    ///
    /// Requires a [`SchemaValidator`](mez_schema::SchemaValidator).
    pub fn validate_schema(&self, validator: &mez_schema::SchemaValidator) -> Result<(), VcError> {
    ...
    pub fn compute_asset_id(genesis: &serde_json::Value) -> Result<String, VcError> {
        ...
        let digest = mez_crypto::sha256_digest(&canonical);
        Ok(digest.to_hex())
    }
...
mod tests {
    ...
    fn registry_vc_json_matches_python_structure() {
        ...
        assert_eq!(
            val["type"],
            json!(["VerifiableCredential", "MezSmartAssetRegistryCredential"])
        );
