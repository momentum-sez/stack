...
module.exports = function build_appendixK() {
  return [
    ...
    table(
      ...
      [
        ["1.1", "Core infrastructure deployed (compute, storage, network)", "mez verify --all passes"],
        ...
        ["2.1", "Pack Trilogy imported for national law", "mez pack verify --all passes"],
    ...
    table(
      ...
      [
        ["1.1", "Infrastructure availability", ">= 99.9% uptime over 72-hour burn-in", "Synthetic health checks every 30s; downtime = any period where mez verify --all fails"],
        ["1.1", "API response latency (p99)", "<= 200ms for all EZ Stack endpoints", "Load test with 100 concurrent users for 1 hour; measure p99 from Axum access logs"],
        ...
        ["2.1", "Schema validation pass rate", "100% of pack documents valid", "Run mez pack verify --all; zero validation failures"],
        ...
        ["2.3", "First corridor activation", "PAK-UAE corridor in Active state", "mez corridor status --id PAK-UAE returns Active; definition and agreement VCs verified"],
        ...
        ["3.2", "Tax gap reduction", ">= 15% reduction vs. baseline", "Compare pre-deployment vs. 6-month post-deployment tax collection for EZ entities"],
        ["3.2", "Filing compliance rate", ">= 90% on-time filing", "Measure percentage of EZ entities filing returns by deadline vs. pre-deployment baseline"],
    ...
    new Paragraph({ alignment: AlignmentType.CENTER, spacing: { after: 60 }, children: [
      new TextRun({ text: "Momentum Open Source EZ Stack", font: C.BODY_FONT, size: C.BODY_SIZE, bold: true, color: C.H1_COLOR })
    ]}),
    new Paragraph({ alignment: AlignmentType.CENTER, spacing: { after: 60 }, children: [
