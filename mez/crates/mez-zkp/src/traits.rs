...
//! the `mez-zkp` crate can exist. External crates cannot implement it. This
...
///
/// Only types within `mez-zkp` that implement `private::Sealed` can
/// implement `ProofSystem`. This prevents external crates from creating
/// unauthorized proof backends.
mod private {
    /// Sealing marker trait. Not accessible outside `mez-zkp`.
    pub trait Sealed {}
}
