...
///
/// ```text
/// proof = SHA256( public_inputs )
/// ```
///
/// Circuit data is validated for canonicalizability (no floats) but is not
/// included in the proof hash. In a real ZK system, the circuit is bound
/// to the proof through the verifying key during circuit setup. Since the
/// mock system uses stateless keys, circuit binding is not available.
///
/// ## Verification
///
/// ```text
...
impl ProofSystem for MockProofSystem {
    ...
    ///
    /// Validates `circuit.circuit_data` for canonicalizability (rejects floats),
    /// then computes `SHA256(circuit.public_inputs)`. Circuit data is validated
    /// but not included in the proof hash — in a real ZK system, circuit binding
    /// happens through the verifying key, not the proof hash.
    fn prove(
        &self,
        _pk: &Self::ProvingKey,
...
mod tests {
    ...
    #[test]
    fn same_public_inputs_produce_same_proof_regardless_of_circuit_data() {
