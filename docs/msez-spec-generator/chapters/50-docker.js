...
module.exports = function build_chapter50() {
  return [
    ...
    p("Docker Compose orchestrates four services with dependency ordering and health checks. The architecture consolidates all five primitive API services into a single msez-api binary (Axum), with PostgreSQL for persistence and Prometheus/Grafana for observability. This replaces the prior nine-service Python layout where individual services could not start because their CLI subcommands did not exist."),
    table(
      ["Service", "Image", "Port", "Function"],
      [
        ["msez-api", "Built from deploy/docker/Dockerfile", "8080", "Single Axum HTTP server exposing all endpoints: corridor operations, smart asset lifecycle, compliance evaluation, credentials, identity, tax, GovOS, settlement, agentic, and regulator console"],
        ...
        ["prometheus", "prom/prometheus:v2.51.0", "9090", "Metrics collection from msez-api /metrics endpoint. 30-day retention with lifecycle management enabled"],
    ...
    p("The Dockerfile uses a two-stage build: compilation in a rust:1.77-bookworm image and runtime on debian:bookworm-slim. Both the msez-api server and msez-cli tool are compiled and included in the final image."),
    ...codeBlock(
...
COPY crates/ ./crates/
RUN cargo build --release -p msez-api -p msez-cli

# Stage 2: Runtime — minimal production image
FROM debian:bookworm-slim
RUN apt-get update && apt-get install -y --no-install-recommends \\
    ca-certificates libssl3 libpq5 curl && rm -rf /var/lib/apt/lists/*
RUN groupadd -r msez && useradd -r -g msez -d /app -s /sbin/nologin msez
COPY --from=builder /app/target/release/msez-api /usr/local/bin/
COPY --from=builder /app/target/release/msez-cli /usr/local/bin/
COPY modules/ /app/modules/
COPY schemas/ /app/schemas/
COPY jurisdictions/ /app/jurisdictions/
RUN mkdir -p /app/data /app/config && chown -R msez:msez /app
WORKDIR /app
USER msez
EXPOSE 8080
...
CMD ["msez-api"]`
    ...
    p_runs([bold("Security Hardening. "), "The runtime container runs as a non-root user (msez). The HEALTHCHECK directive uses curl against /health/liveness to detect unresponsive containers. Resource limits are enforced at the Docker Compose level to allow per-deployment tuning."]),
  ];
};
