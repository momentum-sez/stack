...
// ===========================================================================
// Jurisdiction-Specific Regpack Content Modules
// ===========================================================================
...
// ===========================================================================
// Multi-Jurisdiction Regpack Registry
// ===========================================================================
//
// Provides a unified dispatch interface for building regpacks across all
// jurisdictions with content. This replaces the previous Pakistan-only
// hardcoded path and makes the "AWS of Zones" pattern complete: any zone
// can build its regulatory packs from the same CLI.

/// Jurisdiction regpack content descriptor — enumerates available domains
/// for a given jurisdiction.
pub struct JurisdictionRegpackInfo {
    /// Jurisdiction identifier (e.g., "pk", "ae", "sg").
    pub jurisdiction_id: &'static str,
    /// Human-readable jurisdiction name.
    pub jurisdiction_name: &'static str,
    /// Available regpack domains for this jurisdiction.
    pub available_domains: &'static [&'static str],
}
...
/// Look up available domains for a jurisdiction.
///
/// Returns `None` if the jurisdiction has no regpack content.
pub fn domains_for_jurisdiction(jurisdiction: &str) -> Option<Vec<&'static str>> {
    available_jurisdictions()
        .into_iter()
        .find(|j| j.jurisdiction_id == jurisdiction)
...
/// Build any regpack domain artifact for a given jurisdiction.
///
/// Dispatches to the appropriate domain-specific builder.
pub fn build_regpack_artifact(jurisdiction: &str, domain: &str) -> PackResult<(String, Vec<u8>)> {
    match domain {
        "financial" => build_financial_regpack_artifact(jurisdiction),
        "sanctions" => build_sanctions_regpack_artifact(jurisdiction),
        _ => Err(PackError::Validation(format!(
            "Unknown regpack domain '{domain}'. Available domains: financial, sanctions"
        ))),
    }
}
