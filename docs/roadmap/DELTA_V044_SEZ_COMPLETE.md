# v0.4.44 DELTA: Completing the Special Economic Zone in a Box
...
**Prime Directive for v0.4.44:** Close every gap required so that `git clone` + `msez deploy --profile digital-financial-center` produces a zone with functioning corporate services, banking, payments, custody, arbitration, licensing, identity, and the beginnings of capital markets and trade finance.
...
**Why critical:** No SEZ operates without a fiscal framework — even tax-free zones have fee structures, and international operations require tax treaty awareness.
...
**Why critical:** SEZs exist primarily to facilitate trade. Trade playbook schemas exist but aren't formalized as modules.
...
| 5 | Zone provisioning CLI (`msez zone create`) | 1 command | ~600 Python |
...
### Functional
- [ ] A new entity can be formed in the zone via `msez entity create`
...
- [ ] `msez zone create --profile digital-financial-center` generates a deployable zone repo
