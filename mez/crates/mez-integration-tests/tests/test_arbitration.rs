...
use mez_core::{sha256_digest, CanonicalBytes, CorridorId, Did, JurisdictionId, Timestamp};
use serde_json::json;
...
fn make_digest(label: &str) -> mez_core::ContentDigest {
...
fn dispute_evidence_chain_of_custody() {
    ...
    let dispute_id = mez_arbitration::DisputeId::new();
...
fn enforcement_order_preconditions() {
    ...
    let mut order = EnforcementOrder::new(
        ...
        vec![EnforcementAction::EscrowRelease {
            escrow_id: mez_arbitration::EscrowId::new(),
...
fn terminal_state_rejects_all_transitions() {
    ...
    dispute
        .settle(mez_arbitration::SettlementEvidence {
