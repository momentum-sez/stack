...
use mez_vc::proof::{Proof, ProofPurpose, ProofType};

// =========================================================================
// mez-arbitration types
// =========================================================================

use mez_arbitration::dispute::{DisputeId, DisputeState, DisputeType};
use mez_arbitration::enforcement::{EnforcementOrderId, EnforcementReceiptId, EnforcementStatus};
use mez_arbitration::escrow::{
...
use mez_agentic::audit::{AuditEntry, AuditEntryType};
use mez_agentic::policy::{
    AuthorizationRequirement, Condition, ImpactLevel, PolicyAction, TriggerType,
};
use mez_agentic::scheduler::ActionStatus;

// =========================================================================
// mez-tensor types
// =========================================================================

use mez_tensor::evaluation::{AttestationRef, ComplianceState};
use mez_tensor::tensor::TensorCell;
...
fn test_digest() -> ContentDigest {
    ...
    mez_core::sha256_digest(&canonical)
}

// =========================================================================
// mez-core serde round-trips
// =========================================================================
...
// =========================================================================
// mez-corridor serde round-trips
// =========================================================================
...
fn serde_rt_verifiable_credential_unsigned() {
    ...
    let original = VerifiableCredential {
        ...
        id: Some("urn:mez:vc:test:serde-001".to_string()),
...
fn serde_rt_verifiable_credential_with_expiration() {
    let original = VerifiableCredential {
        ...
        id: Some("urn:mez:vc:test:expiry".to_string()),
...
// =========================================================================
// mez-arbitration serde round-trips
// =========================================================================
...
fn serde_rt_did_rejects_invalid() {
    ...
    let result: Result<mez_core::Did, _> = serde_json::from_str(invalid_json);
    assert!(
        result.is_err(),
...
fn serde_rt_ntn_rejects_invalid() {
    ...
    let result: Result<mez_core::Ntn, _> = serde_json::from_str(invalid_json);
    assert!(
        result.is_err(),
...
fn serde_rt_cnic_rejects_invalid() {
    ...
    let result: Result<mez_core::Cnic, _> = serde_json::from_str(invalid_json);
    assert!(
        result.is_err(),
...
fn serde_rt_passport_number_rejects_invalid() {
    ...
    let result: Result<mez_core::PassportNumber, _> = serde_json::from_str(invalid_json);
    assert!(
        result.is_err(),
...
// =========================================================================
// mez-arbitration: Full struct round-trip tests
// =========================================================================

use mez_arbitration::dispute::{ArbitrationInstitution, Claim, Dispute, Money, Party as ArbParty};
use mez_arbitration::enforcement::{EnforcementAction, EnforcementOrder, EnforcementPrecondition};
use mez_arbitration::escrow::{EscrowAccount, EscrowTransaction, ReleaseCondition};
use mez_arbitration::evidence::{
...
// =========================================================================
// mez-agentic: Condition variant exhaustive round-trips
// =========================================================================
...
// =========================================================================
// mez-agentic: ScheduledAction and CronSchedule round-trips
// =========================================================================

use mez_agentic::scheduler::{CronSchedule, SchedulePattern, ScheduledAction};
...
// =========================================================================
// mez-corridor: Anchor and Fork types round-trips
// =========================================================================

use mez_corridor::anchor::{AnchorCommitment, AnchorStatus};
use mez_corridor::fork::ResolutionReason;
...
// =========================================================================
// mez-corridor: CorridorReceipt and Checkpoint round-trips
// =========================================================================

use mez_corridor::receipt::CorridorReceipt;
...
// =========================================================================
// mez-state: DissolutionStage round-trips
// =========================================================================

use mez_state::DissolutionStage;
...
// =========================================================================
// mez-pack: Lawpack types serde round-trips
// =========================================================================

use mez_pack::lawpack::{
...
// =========================================================================
// mez-pack: Regpack types serde round-trips
// =========================================================================
...
// =========================================================================
// mez-pack: Licensepack types serde round-trips
// =========================================================================

use mez_pack::licensepack::{
...
// =========================================================================
// mez-mass-client: Mass DTO serde round-trips
// =========================================================================

use mez_mass_client::consent::{MassConsent, MassConsentOperationType, MassConsentStatus};
use mez_mass_client::entities::{MassEntity, MassEntityStatus, MassEntityType};
use mez_mass_client::fiscal::{MassFiscalAccount, MassPayment, MassPaymentStatus, MassTaxEvent};
use mez_mass_client::identity::{
    MassIdentity, MassIdentityStatus, MassIdentityType, MassMember, MassShareholder,
};
use mez_mass_client::ownership::{MassCapTable, MassOwnershipTransfer, MassShareClass};
...
use mez_zkp::circuits::settlement::{
    MerkleMembershipCircuit, NettingValidityCircuit, RangeProofCircuit,
};
use mez_zkp::mock::{MockCircuit, MockProof};
...
// =========================================================================
// mez-arbitration: Complex struct serde round-trips
// =========================================================================
...
fn serde_rt_dispute_full_roundtrip() {
    let original = Dispute {
        ...
        claimant: ArbParty {
            did: mez_core::Did::new("did:key:z6MkClaimant").unwrap(),
            legal_name: "Claimant Corp".to_string(),
            jurisdiction_id: Some(JurisdictionId::new("PK").unwrap()),
        },
        respondent: ArbParty {
            did: mez_core::Did::new("did:key:z6MkRespondent").unwrap(),
...
fn serde_rt_enforcement_order_full_roundtrip() {
    use mez_arbitration::enforcement::EnforcementAction;
    let original = EnforcementOrder::new(
        DisputeId::new(),
        test_digest(),
        vec![EnforcementAction::MonetaryPenalty {
            party: mez_core::Did::new("did:key:z6MkPenalized").unwrap(),
...
fn serde_rt_chain_of_custody_entry_roundtrip() {
    let original = ChainOfCustodyEntry {
        custodian: mez_core::Did::new("did:key:z6MkCustodian").unwrap(),
...
// =========================================================================
// mez-corridor: Untested types
// =========================================================================

use mez_corridor::anchor::AnchorReceipt;
use mez_corridor::fork::ForkBranch;
use mez_corridor::receipt::Checkpoint;
...
fn serde_rt_checkpoint_full() {
    let original = Checkpoint {
        ...
        ruleset_digest_set: Vec::new(),
        mmr: mez_corridor::MmrCommitment {
