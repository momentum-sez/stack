...
module.exports = function build_chapter42() {
  return [
    ...
    p("Mass Protocol provides the settlement layer for the EZ Stack. The protocol architecture comprises: transaction layer (private and public transaction types), consensus layer (DAG-based with jurisdictional awareness), proving layer (Groth16 + Plonk backends with recursive proof aggregation), and anchoring layer (periodic state commitment to external chains)."),
    p("Integration patterns between the MEZ Stack and Mass Protocol follow the anchor-and-verify model: MEZ Stack operations produce receipts, receipts aggregate into checkpoints, checkpoints anchor to Mass Protocol periodically, and protocol provides finality guarantees for anchored state."),
