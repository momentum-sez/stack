...
//! 1. Compile-time feature flags (`--cfg mez_production`)
//! 2. Runtime environment variable (`MEZ_PROOF_POLICY`)
...
//! ```text
//! cargo test --release --package mez-zkp -- policy::tests::release_build_rejects_mock
//! ```
...
///
/// ```rust,no_run
/// use mez_zkp::policy::{ProofPolicy, ProofBackend, PolicyMode};
///
...
mod tests {
    ...
    /// Run with: `cargo test --release --package mez-zkp -- policy::tests::release_build_rejects_mock`
    #[test]
    fn release_build_rejects_mock() {
        if cfg!(not(debug_assertions)) {
            ...
            if std::env::var("MEZ_PROOF_POLICY").is_err() {
