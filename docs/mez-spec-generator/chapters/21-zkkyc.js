...
module.exports = function build_chapter21() {
  return [
    ...
    p_runs([bold("Step 1 -- Credential Issuance. "), "The entity completes traditional KYC/KYB verification with an approved issuer (e.g., NADRA for Pakistan CNIC, or an accredited KYC provider for international passports). The issuer produces a signed attestation containing the entity's identity details, KYC tier, jurisdiction, and expiry date. This attestation is delivered exclusively to the entity and anchored as a W3C Verifiable Credential via mez-vc."]),
    p_runs([bold("Step 2 -- Commitment Registration. "), "The entity computes a commitment over the attestation using CanonicalBytes and registers the commitment digest with the EZ Stack. The commitment is bound to the entity's DID. The attestation itself is never transmitted to the EZ Stack."]),
    ...
    p_runs([bold("Step 4 -- On-Chain Verification. "), "The verifier (corridor counterparty, regulator endpoint, or the EZ Stack API itself) checks the ZK proof against the public inputs. Verification is constant-time and requires no access to the underlying identity data. The proof is anchored to the receipt chain for audit trail purposes."]),
    p_runs([bold("Step 5 -- Selective Disclosure (Optional). "), "If a regulator presents a valid legal instrument (subpoena, court order, regulatory examination notice), the entity can produce a BBS+ selective disclosure proof from the original Verifiable Credential, revealing only the specific fields required by the instrument. This is mediated through the consent primitive via mez-mass-client."]),
    ...
    table(
      ...
      [
        ...
        [
          "Entity formation",
          "KYC tier >= Tier 2, jurisdiction = target EZ, credential non-expiry",
