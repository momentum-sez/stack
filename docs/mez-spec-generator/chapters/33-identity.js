const {
  ...
  p, p_runs, bold, pageBreak, table, codeBlock
...
module.exports = function build_chapter33() {
  return [
    ...
    p("The identity system supports four DID methods, each serving a distinct operational context. Method selection is determined by the jurisdiction's requirements, the entity type, and the deployment environment. All methods produce DIDs that can anchor Verifiable Credentials issued by mez-vc."),
    table(
      ["DID Method", "Format", "Description", "Use Case"],
      [
        ["did:mez", "did:mez:<zone>:<id>", "Native Mass/EZ method anchored to the corridor receipt chain. Provides the strongest binding to jurisdictional context and compliance state. Resolution requires access to the EZ Stack.", "Primary method for all entities formed through the EZ Stack. Required for corridor participation and compliance attestation."],
        ...
        ["did:ion", "did:ion:<long-form>", "Decentralized DID anchored to a Layer 1 blockchain via the ION network. Provides maximum censorship resistance and long-term persistence independent of any single operator.", "Long-lived identities requiring maximum decentralization, cross-platform portability, and independence from EZ Stack availability."],
      ],
      [1400, 1800, 3560, 2600]
    ),
    ...
    p("Each KYC tier grants a specific set of operational capabilities within the EZ Stack. The compliance tensor evaluates tier requirements per jurisdiction, meaning the same tier may grant different capabilities depending on the applicable regulatory framework."),
    table(
