...
mod tests {
    ...
    async fn credential_contains_correct_issuer_and_type() {
        ...
        assert!(type_strs.contains(&"MezComplianceAttestationCredential"));

        // Verify ID format.
        let vc_id = credential["id"].as_str().unwrap();
        assert!(
            vc_id.starts_with("urn:mez:vc:compliance:"),
