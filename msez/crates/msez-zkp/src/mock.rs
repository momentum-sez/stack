...
impl ProofSystem for MockProofSystem {
    ...
    fn prove(
        ...
        // Canonicalize circuit data to validate it (rejects floats, ensures
        // deterministic bytes), but do NOT include it in the proof hash.
        // The proof hash covers only public_inputs so that verify() — which
        // has no access to circuit_data — can recompute the same digest.
        ...
        // SHA256(public_inputs) — symmetric with verify().
...
impl MockProofSystem {
    ...
    pub fn verify_circuit(
        ...
        // Validate circuit data can be canonicalized (same check as prove()).
        let _canonical = CanonicalBytes::from_value(circuit.circuit_data.clone()).map_err(|e| {
            VerifyError::MalformedProof(format!("failed to canonicalize circuit data: {e}"))
        })?;

        // Verify using only public_inputs — symmetric with prove().
        self.verify(vk, proof, &circuit.public_inputs)
    }
}
...
mod tests {
    ...
    #[test]
    fn same_public_inputs_same_proof_regardless_of_circuit_data() {
        // prove() hashes only public_inputs (symmetric with verify()).
        // Different circuit_data with same public_inputs → same proof.
        let (sys, pk, _vk) = make_system();
    ...
    fn prove_then_verify_roundtrip() {
        // prove() and verify() are now symmetric: both hash only public_inputs.
        ...
        let proof = sys.prove(&pk, &circuit).unwrap();
        // verify() with the same public_inputs should succeed.
        let result = sys.verify(&vk, &proof, &circuit.public_inputs).unwrap();
        assert!(
            result,
