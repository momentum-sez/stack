...
//! ## Ported from Python
//!
//! This module replaces `tools/mez/composition.py` (P1-006).
...
use mez_core::{CanonicalBytes, ComplianceDomain};
...
/// use mez_pack::composition::JurisdictionLayer;
///
...
///
/// ```
/// use mez_pack::composition::ZoneCompositionBuilder;
/// use mez_core::ComplianceDomain;
///
...
pub fn load_composition_from_value(data: &serde_json::Value) -> PackResult<ZoneComposition> {
    ...
    let zone_id = obj
        ...
        .ok_or_else(|| PackError::SchemaViolation {
            message: "zone_id must be a non-empty string".to_string(),
        })?
        .to_string();
...
fn parse_layer(value: &serde_json::Value) -> PackResult<JurisdictionLayer> {
    ...
    let jurisdiction_id = obj
        ...
        .filter(|s| !s.trim().is_empty())
        .ok_or_else(|| PackError::SchemaViolation {
            message: "layer jurisdiction_id must be a non-empty string".to_string(),
        })?
        .to_string();
