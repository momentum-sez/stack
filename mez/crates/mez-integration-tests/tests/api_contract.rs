...
async fn create_corridor(app: &axum::Router) -> String {
    let resp = app
        ...
        .oneshot(post_json(
            "/v1/corridors",
            json!({"jurisdiction_a": "PK-PEZ", "jurisdiction_b": "AE-DIFC"}),
        ))
        .await
        .unwrap();
...
async fn create_asset(app: &axum::Router) -> String {
    let resp = app
        ...
        .oneshot(post_json(
            "/v1/assets/genesis",
            json!({"asset_type": "equity", "jurisdiction_id": "PK-PEZ"}),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), StatusCode::CREATED);
...
async fn corridor_create_empty_jurisdiction_b() {
    ...
    let resp = app
        .oneshot(post_json(
            "/v1/corridors",
            json!({"jurisdiction_a": "PK-PEZ", "jurisdiction_b": ""}),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), StatusCode::UNPROCESSABLE_ENTITY);
...
async fn corridor_create_same_jurisdictions() {
    ...
    let resp = app
        .oneshot(post_json(
            "/v1/corridors",
            json!({"jurisdiction_a": "PK-PEZ", "jurisdiction_b": "PK-PEZ"}),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), StatusCode::UNPROCESSABLE_ENTITY);
...
async fn asset_create_empty_asset_type() {
    ...
    let resp = app
        .oneshot(post_json(
            "/v1/assets/genesis",
            json!({"asset_type": "", "jurisdiction_id": "PK-PEZ"}),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), StatusCode::UNPROCESSABLE_ENTITY);
...
async fn asset_create_asset_type_too_long() {
    ...
    let resp = app
        .oneshot(post_json(
            "/v1/assets/genesis",
            json!({"asset_type": long, "jurisdiction_id": "PK-PEZ"}),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), StatusCode::UNPROCESSABLE_ENTITY);
                ...
                "legs": [
                    {
                        ...
                        "from_bic": "MEZPK33",
                        "to_bic": "MEZAE33",
                        "from_account": "PK00MEZ0001",
                        "to_account": "AE00MEZ0001"
                    }
                ]
            }),
        ))
...
async fn asset_genesis_null_type() {
    ...
    let resp = app
        .oneshot(post_json(
            "/v1/assets/genesis",
            json!({"asset_type": null, "jurisdiction_id": "PK-PEZ"}),
        ))
        .await
        .unwrap();
