...
module.exports = function build_executive_summary() {
  return [
    ...
    p("Economic zones have existed since antiquity \u2014 from Phoenician free ports to the Shannon Free Zone of 1959 to the 5,400+ special economic zones operating worldwide today. Every one of them was built the same way: years of legislative drafting, months of institutional negotiation, bespoke IT procurement, and manual regulatory compliance. The Momentum Open Source SEZ Stack eliminates that entire process. Version 0.4.44, codenamed GENESIS, is the first open-source software system that allows a sovereign to instantiate a fully functional, cryptographically auditable, compliance-enforcing Special Economic Zone from a single deployment command."),

    p("GENESIS represents a categorical shift. Prior versions of the Stack provided execution infrastructure \u2014 compliance tensors, corridor state machines, verifiable credentials. Version 0.4.44 assembles these primitives into a complete jurisdictional operating system. It adds institutional infrastructure (corporate services, identity, tax, capital markets, trade), a four-layer GovOS architecture for sovereign deployment, a multi-jurisdiction composition engine, and production-grade deployment tooling. The result is a system that can be cloned, configured for a target jurisdiction, and deployed \u2014 yielding a zone that processes entity formation, enforces tax law, clears securities, manages trade corridors, and produces cryptographic compliance attestations."),

    p("This specification documents the complete technical architecture of the SEZ Stack in 56 chapters across 18 Parts, plus 11 appendices. The codebase is fully Rust (2024 edition). All code examples, data structures, and system interfaces are specified in Rust. The architecture enforces a strict separation between two systems: Mass (five jurisdiction-agnostic programmable primitives deployed as live production APIs) and the MSEZ Stack (the jurisdictional orchestration layer that makes those primitives compliance-aware). This separation is the central architectural invariant of the system."),

    p("The specification is grounded in production deployments processing real capital across real jurisdictions: Pakistan GovOS covering 40+ ministries with FBR tax integration, SBP Raast payments, NADRA identity verification, and SECP corporate registry; UAE/ADGM with 1,000+ entities onboarded and $1.7B+ capital processed; Dubai Free Zone Council integration across 27 free zones; Kazakhstan Alatau City SEZ + AIFC composition engine; and three cross-border trade corridors (PAK\u2194KSA $5.4B, PAK\u2194UAE $10.1B, PAK\u2194CHN $23.1B). These are not hypothetical deployments. They are the empirical foundation on which every design decision in this document rests."),
