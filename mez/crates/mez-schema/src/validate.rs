...
//! `https://schemas.momentum-ez.org/mez/{name}` to local schema files.
//!
//! ## Design
//!
...
/// URI prefix used by all MEZ schemas.
const SCHEMA_URI_PREFIX: &str = "https://schemas.momentum-ez.org/mez/";

/// A retriever that resolves `$ref` URIs by looking up pre-loaded schemas.
///
/// All MEZ schemas use `$id` values of the form
/// `https://schemas.momentum-ez.org/mez/{filename}`. This retriever maps
...
/// A compiled schema validator that validates documents against the MEZ
/// JSON Schema corpus.
///
...
impl SchemaValidator {
    ...
    ///
    /// This is the Rust equivalent of `tools/mez/schema.py:validate_module`.
    ...
    /// of each failure.
    ///
    /// This matches the behavior of `mez validate --all-modules` from the
    /// Python CLI.
...
mod tests {
    ...
    fn repo_root() -> PathBuf {
        ...
        // mez/crates/mez-schema -> mez -> stack
        dir.pop(); // -> mez/crates
        dir.pop(); // -> mez
    ...
    fn test_schema_validator_from_nonexistent_dir() {
        let validator = SchemaValidator::new("/tmp/definitely-not-a-real-dir-mez-test-12345");
    ...
    fn test_validate_module_nonexistent_path() {
        let validator = SchemaValidator::new(schema_dir()).expect("failed to load schemas");
        let result = validator.validate_module(Path::new("/tmp/no-such-module-dir-mez-12345"));
        assert!(matches!(
            result,
    ...
    fn test_validate_zone_nonexistent_path() {
        let validator = SchemaValidator::new(schema_dir()).expect("failed to load schemas");
        let result = validator.validate_zone(Path::new("/tmp/no-such-zone-mez-12345.yaml"));
        assert!(matches!(
            result,
            Err(SchemaValidationError::DocumentLoadError { .. })
        ));
    ...
    fn test_validate_profile_nonexistent_path() {
        let validator = SchemaValidator::new(schema_dir()).expect("failed to load schemas");
        let result = validator.validate_profile(Path::new("/tmp/no-such-profile-mez-12345.yaml"));
        assert!(matches!(
            result,
            Err(SchemaValidationError::DocumentLoadError { .. })
        ));
    ...
    fn test_validation_error_display() {
        let detail = SchemaValidationDetail {
            schema_path: "https://schemas.momentum-ez.org/mez/module.schema.json".to_string(),
