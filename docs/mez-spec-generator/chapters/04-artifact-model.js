...
module.exports = function build_chapter04() {
  return [
    ...
    // --- 4.1 Digest Type ---
    h2("4.1 Digest Type"),
    p("Every artifact in the MEZ Stack is content-addressed. The artifact identifier is its cryptographic digest. This provides integrity (modification changes the identifier), deduplication (identical content shares an identifier), and auditability (any party can verify artifact integrity)."),
    ...
    ...codeBlock(
      ...
      "mez artifact graph verify transition-types <digest> --strict --json\n" +
      "\n" +
      "# Generate witness bundle for offline transfer\n" +
      "mez artifact graph verify transition-types <digest> --bundle /tmp/witness.zip\n" +
      "\n" +
      "# Attest (sign) a witness bundle for provenance\n" +
      "mez artifact bundle attest /tmp/witness.zip \\\n" +
    ...
    ...codeBlock(
      ...
      "mez artifact list --type lawpack\n" +
      "\n" +
      "# Verify integrity of all artifacts in the CAS tree\n" +
      "mez artifact verify --all --strict\n" +
      "\n" +
      "# Import an artifact into the CAS from an external source\n" +
      "mez artifact import ./pk-ito-2001.zip --type lawpack"
    ),
