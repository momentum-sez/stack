...
- `mez proof-binding init`
  - Create a `MEZProofBinding` object, compute semantic digest (proof excluded), and optionally store in the generic artifact CAS.
- `mez proof-binding verify`
  - Validate schema + recompute digest; optionally verify Ed25519 proofs.

### Settlement anchors

- `mez corridor settlement-anchor-init`
  - Create a `MEZCrossCorridorSettlementAnchor` artifact binding an obligation corridor checkpoint to a settlement corridor checkpoint.
  - Optionally include proof-bindings and/or raw proof blobs as ArtifactRefs.
  - Optionally store in the artifact CAS.
- `mez corridor settlement-anchor-verify`
  - Validate schema + recompute digest; optionally verify Ed25519 proofs and best-effort check embedded ArtifactRef uris.

### Corridor receipts

`mez corridor state receipt-init` now supports additional typed attachments:
...
- Stack spec version bump: **0.4.36 → 0.4.38**
