...
impl ProofSystem for MockProofSystem {
    ...
    ///
    /// **Important:** For mock proofs generated by `prove()`, the `public_inputs`
    /// passed here must be `canonical_bytes(circuit_data) || original_public_inputs`
    /// to match the hash computed during proof generation.
    ///
    /// # SHA-256 exception: mock proof verification
    ///
    /// Uses `msez_core::sha256_raw` to recompute the expected digest from raw
    /// binary public inputs. Same rationale as `prove()` above.
    fn verify(
        &self,
