# Architecture Audit Findings — EZ Stack

**Date**: 2026-02-15
**Scope**: Full structural integrity audit of `momentum-ez/stack` Rust workspace
...
The EZ Stack codebase is in significantly better shape than the defect inventory
in CLAUDE.md v5.0 suggests. Key findings:

1. **Zero production-code `.unwrap()` calls** across all priority crates (mez-api,
   mez-crypto, mez-core). The ~2,200 unwrap count in the defect inventory
...
| Invariant | Status |
|-----------|--------|
| `mez-core` has zero internal dependencies | **PASS** — depends only on serde, serde_json, thiserror, chrono, uuid, sha2 |
| `mez-mass-client` has zero internal dependencies | **PASS** — zero mez-* dependencies (better than required) |
| No cycles in dependency graph | **PASS** — DAG verified |
| `mez-api` is sole composition crate | **PASS** — no other crate depends on mez-api |
| SHA-256 through CanonicalBytes | **PASS** — structural enforcement via mez-core |
| ComplianceDomain defined once in mez-core | **PASS** — 20 variants, single definition |

**Dependency tree (leaf → root):**

```
mez-core (foundation, zero deps)
├── mez-crypto → mez-core
├── mez-state → mez-core
├── mez-pack → mez-core
├── mez-schema → mez-core
├── mez-agentic → mez-core
├── mez-tensor → mez-core, mez-crypto
├── mez-arbitration → mez-core, mez-state
├── mez-corridor → mez-core, mez-crypto, mez-state
├── mez-vc → mez-core, mez-crypto, mez-schema
├── mez-zkp → mez-core, mez-crypto
├── mez-compliance → mez-core, mez-tensor, mez-pack
├── mez-mass-client (zero internal deps)
├── mez-cli → mez-core, mez-crypto, mez-pack, mez-schema, mez-state
└── mez-api (composition root) → 10 crates
```
...
|-------|---------------|--------------------------|------------------|
| mez-api | 391 | **0** | Yes |
| mez-crypto | 161 | **0** | Yes |
| mez-core | 141 | **0** | Yes |

Every handler in `mez-api/src/routes/` uses:
...
|----|-------------|--------|----------|
| P0-001 | Zeroize on Ed25519 signing key | **RESOLVED** | `Zeroize` impl + `Drop` impl in `mez-crypto/src/ed25519.rs:129-195` |
| P0-002 | Constant-time token comparison | **RESOLVED** | `subtle::ConstantTimeEq` in `mez-api/src/auth.rs:164-173` with dummy comparison on length mismatch |
| P0-003 | Lock poisoning prevention | **RESOLVED** | `parking_lot::RwLock` everywhere in `mez-api/src/state.rs`. Zero instances of `std::sync::RwLock` |
...
- `MezError` (mez-core): 10 variants covering canonicalization, state transition,
...
- `AppError` (mez-api): HTTP-specific wrapper with proper status code mapping.
  Includes structured logging at appropriate levels per variant.

- `CryptoError` (mez-crypto): 7 variants for crypto-specific failures.

- `ValidationError` (mez-core): 6 variants for domain primitive validation with
...
P2-001 in CLAUDE.md claimed `MezError::NotImplemented` is unused. This is
**incorrect** — it is actively used in:

| Location | Endpoint | Purpose |
|----------|----------|---------|
| `mez-api/src/routes/mass_proxy.rs:268` | PUT /v1/entities/:id | Entity update proxy (Phase 2) |
| `mez-api/src/routes/corridors.rs:562` | POST /v1/corridors/anchor | L1 anchor commitment (Phase 2) |
| `mez-api/src/routes/corridors.rs:579` | GET /v1/corridors/anchor/status | Finality status (Phase 2) |
| `mez-api/src/routes/smart_assets.rs:172` | POST /v1/assets/:id/registry | Registry submission (Phase 2) |
| `mez-api/src/routes/smart_assets.rs:283` | POST /v1/assets/:id/anchor/verify | Anchor verification (Phase 2) |
| `mez-crypto/src/poseidon.rs:63,75` | Poseidon2 hash/verify | ZKP primitive (stub) |
| `mez-crypto/src/bbs.rs:95,113,130` | BBS+ sign/verify/derive | Selective disclosure (stub) |
| `mez-zkp/src/groth16.rs:89,100` | Groth16 prove/verify | ZKP backend (stub) |
| `mez-zkp/src/plonk.rs:90,101` | PLONK prove/verify | ZKP backend (stub) |
| `mez-core/src/digest.rs:159` | Poseidon bridge digest | CDB computation (stub) |
...
| P1-006 | Composition engine Python-only | **RESOLVED** | Ported to `mez-pack/src/composition.rs`; Python removed |
| P1-007 | CLI commands Python-only | **RESOLVED** | Python CLI removed; core commands ported to `mez-cli` |
...
| ID | Description | Status | Notes |
|----|-------------|--------|-------|
| P2-001 | MezError::NotImplemented unused | **INCORRECT** | Actively used in 10+ locations (see §1E above) |
| P2-002 | mez-mass-client doesn't share types with mez-core | **BY DESIGN** | mez-mass-client has zero internal deps, which is stricter than required |
...
| 4 | Zero `unwrap()` in mez-api request paths | **PASS** |
| 5 | Zero `unwrap()` in mez-crypto non-test code | **PASS** |
| 6 | Zero `unimplemented!()` or `todo!()` in non-test code | **PASS** |
| 7 | mez-mass-client contract tests vs Mass API specs | **NOT YET** (P0-008) |
| 8 | Cross-language parity tests | **PASS** (Python removed; hardcoded vectors retained) |
| 9 | Composition engine in Rust | **PASS** (P1-006 resolved — `mez-pack::composition`) |
| 10 | mass_proxy routes are orchestration | **PASS** (P1-004 resolved — all write endpoints are orchestration pipelines) |
| 11 | Postgres persistence | **PASS** (P1-008 resolved — `mez-api::db` module with SQLx migrations, write-through persistence, startup hydration) |
...
3. Reclassify P2-002 — mez-mass-client having zero internal deps is stricter than
   the invariant requires. This is a feature, not a defect.

### Next Priority (P0-008)

4. Add contract tests for `mez-mass-client` against live Mass API Swagger specs.
