...
pub struct InstructionRequest {
    /// BIC of the instructing agent (default: "MSEZUS33").
...
async fn generate_instructions(
    ...
    for (i, leg) in req.legs.iter().enumerate() {
        ...
        let instruction = SettlementInstruction {
            ...
            amount: leg.amount,
            currency: leg.currency.clone(),
            remittance_info: Some(format!("MSEZ corridor {} settlement", corridor_id)),
        };
...
mod tests {
    ...
    async fn route_through_active_corridors() {
        ...
        create_corridor_in_state(&state, "pk-sez-01", "kz-aifc", DynCorridorState::Halted);
        ...
        // First hop: PK → AE
        assert_eq!(route["hops"][0]["from"], "pk-sez-01");
    ...
    async fn halted_corridors_excluded_from_routing() {
        ...
        create_corridor_in_state(&state, "pk-sez-01", "ae-difc", DynCorridorState::Halted);
    ...
    async fn suspended_corridors_excluded_from_routing() {
        let state = AppState::new();
        create_corridor_in_state(&state, "pk-sez-01", "ae-difc", DynCorridorState::Suspended);
    ...
    fn route_request_validates_different_source_target() {
        let req = RouteRequest {
            source: "pk-sez-01".to_string(),
            target: "pk-sez-01".to_string(),
            default_fee_bps: None,
            default_settlement_time_secs: None,
        };
