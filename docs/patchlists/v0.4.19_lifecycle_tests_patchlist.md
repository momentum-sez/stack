# MEZ Stack v0.4.19 Patchlist — Lifecycle Policy + Fork Tests
...
- `governance/corridor.lifecycle.state-machine.v1.json`
  ...
  - Evidence-gated transitions:
    - `OPERATIONAL → HALTED` requires `MEZCorridorForkAlarmCredential`.
    - `HALTED → OPERATIONAL` requires `MEZCorridorForkResolutionCredential`.
