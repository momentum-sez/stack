...
pub struct VerifiableCredential {
    ...
    p("The Ed25519 proof generation process is the foundation of credential integrity in the EZ Stack. The process enforces type-level security: the SigningKey::sign() method only accepts &CanonicalBytes, making it impossible to sign non-canonicalized data without an explicit unsafe bypass."),
...
pub fn sign_ed25519(
    ...
    p_runs([bold("Proof types."), " The EZ Stack supports two Ed25519 proof type identifiers: ", code("Ed25519Signature2020"), " (W3C standard) and ", code("MezEd25519Signature2025"), " (EZ Stack-specific, for internal interoperability). Both use identical Ed25519 signing mechanics; the type string is purely a namespace identifier. The ", code("BbsBlsSignature2020"), " type is reserved for Phase 2 selective disclosure (Section 43.4)."]),
