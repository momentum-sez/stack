...
use mez_corridor::{AnchorCommitment, AnchorTarget, MockAnchorTarget};
use serde_json::json;
...
fn anchor_commitment_and_verify() {
    ...
    assert_eq!(
        receipt.status,
        mez_corridor::anchor::AnchorStatus::Finalized
    );

    // Verify receipt via check_status
    let status = target.check_status(&receipt.transaction_id).unwrap();
    assert_eq!(status, mez_corridor::anchor::AnchorStatus::Finalized);
}
