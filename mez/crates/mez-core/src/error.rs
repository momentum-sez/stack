//! # Error Hierarchy
//!
//! Structured error types for the entire EZ Stack, built with `thiserror`.
...
/// Top-level error type for the EZ Stack.
#[derive(Error, Debug)]
pub enum MezError {
...
mod tests {
    ...
    #[test]
    fn mez_error_validation_display() {
        let inner = ValidationError::InvalidDid("bad:did".to_string());
        let err = MezError::Validation(inner);
        assert!(format!("{err}").contains("bad:did"));
    }

    #[test]
    fn mez_error_schema_validation_display() {
        let err = MezError::SchemaValidation("missing field".to_string());
        assert!(format!("{err}").contains("missing field"));
    }

    #[test]
    fn mez_error_cryptographic_display() {
        let err = MezError::Cryptographic("bad key".to_string());
        assert!(format!("{err}").contains("bad key"));
    }

    #[test]
    fn mez_error_integrity_display() {
        let err = MezError::Integrity("digest mismatch".to_string());
        assert!(format!("{err}").contains("digest mismatch"));
    }

    #[test]
    fn mez_error_security_display() {
        let err = MezError::Security("injection attempt".to_string());
    ...
    fn all_error_types_are_debug() {
        let e1 = MezError::Security("test".to_string());
