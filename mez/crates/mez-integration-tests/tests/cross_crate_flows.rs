...
use mez_core::{CanonicalBytes, ComplianceDomain, ContentDigest, CorridorId, JurisdictionId};
use mez_crypto::{sha256_digest, ContentAddressedStore, MerkleMountainRange, SigningKey};
...
use mez_state::corridor::{ActivationEvidence, Corridor, Draft, HaltReason, SubmissionEvidence};
...
use mez_tensor::{AttestationRef, ComplianceState, ComplianceTensor, DefaultJurisdiction};
...
fn netting_to_swift_pipeline() {
    ...
    for (i, leg) in plan.settlement_legs.iter().enumerate() {
        ...
        let instruction = SettlementInstruction {
            ...
            remittance_info: Some(format!("EZ Settlement Leg {}", i)),
        };
...
use mez_agentic::{PolicyAction, PolicyEngine, Trigger, TriggerType};
...
fn content_digest_from_core_used_in_crypto_cas() {
    // Verify ContentDigest from mez-core is compatible with mez-crypto CAS
    let tmp = tempfile::tempdir().unwrap();
    let cas = ContentAddressedStore::new(tmp.path());
...
use mez_arbitration::evidence::{EvidenceItem, EvidencePackage, EvidenceType};
use mez_core::Did;
...
fn receipt_chain_checkpoint_stored_in_cas() {
    ...
    let mut receipt = CorridorReceipt {
        ...
        sequence: 0,
        timestamp: mez_core::Timestamp::now(),
        prev_root,
...
use mez_core::MigrationId;
use mez_state::migration::MigrationBuilder;
...
fn policy_engine_corridor_trigger_to_vc() {
    ...
    if actions.iter().any(|a| a.action == PolicyAction::Halt) {
        ...
        let mut vc = VerifiableCredential {
            ...
            issuer: "did:key:z6MkEZ".to_string(),
            issuance_date: chrono::Utc::now(),
            expiration_date: None,
            credential_subject: json!({
        ...
        vc.sign_ed25519(
            &sk,
            "did:key:z6MkEZ#key-1".to_string(),
            ProofType::Ed25519Signature2020,
            None,
        )
        .unwrap();
...
use mez_pack::regpack::validate_compliance_domain;
...
fn netting_to_settlement_to_receipt_chain() {
    ...
    for leg in &plan.settlement_legs {
        let instruction = mez_corridor::swift::SettlementInstruction {
