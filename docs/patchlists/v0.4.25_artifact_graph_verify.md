# v0.4.25 — Artifact graph verification + strict digest recomputation
...
### `mez artifact graph verify`

New command:

```bash
python -m tools.mez artifact graph verify <artifact_type> <digest_sha256> [--store-root <dir> ...] [--strict] [--json] [--out <file>]
python -m tools.mez artifact graph verify --path <local.json|local.yaml> [--store-root <dir> ...] [--strict]
```

Outputs a `MEZArtifactGraphVerifyReport` containing:
