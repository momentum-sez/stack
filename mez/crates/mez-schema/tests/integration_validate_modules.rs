...
//! their JSON schemas.
//!
//! This test matches the behavior of `mez validate --all-modules` from the
//! Python CLI (`tools/mez/schema.py:validate_module`). It:
//!
...
use mez_schema::{SchemaValidationError, SchemaValidator};
use std::path::PathBuf;

/// Compute the repo root from the crate manifest directory.
fn repo_root() -> PathBuf {
    let mut dir = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    // mez/crates/mez-schema -> mez/crates -> mez -> stack (repo root)
    dir.pop();
    dir.pop();
    dir.pop();
    dir
...
fn test_find_all_module_directories() {
    let dirs =
        SchemaValidator::find_all_modules(&modules_dir()).expect("find_all_modules should succeed");
...
fn test_ref_resolution_across_schemas() {
    ...
    let vc = serde_json::json!({
        ...
        "proof": {
            ...
            "verificationMethod": "did:mez:key:test",
            "proofPurpose": "assertionMethod",
            "jws": "test-signature"
        }
    });

    let schema_id = "https://schemas.momentum-ez.org/mez/vc.smart-asset-registry.schema.json";
...
fn test_corridor_receipt_schema_validation() {
    ...
    let schema_id = "https://schemas.momentum-ez.org/mez/corridor.receipt.schema.json";
...
fn test_attestation_schema_validation() {
    ...
    let attestation = serde_json::json!({
        ...
        "issuer": "did:mez:authority:nadra",
        "subject": "did:mez:entity:test",
        ...
        "proof": {
            ...
            "verification_method": "did:mez:key:nadra-01",
            "signature": "base64-signature-data"
        }
    });

    let schema_id = "https://schemas.momentum-ez.org/mez/attestation.schema.json";
...
fn test_codegen_security_critical_schemas_exist() {
    ...
    for filename in mez_schema::SECURITY_CRITICAL_SCHEMAS {
...
fn test_codegen_additional_properties_audit() {
    ...
    for filename in mez_schema::SECURITY_CRITICAL_SCHEMAS {
        if let Some(schema) = validator.get_schema_by_filename(filename) {
            let violations = mez_schema::check_additional_properties_policy(filename, schema);
