...
//! ## Architecture
//!
//! AppState holds only EZ-Stack-owned concerns:
//! - **Corridors** — cross-border corridor lifecycle (EZ Stack domain)
//! - **Smart Assets** — smart asset lifecycle (EZ Stack domain)
//! - **Attestations** — compliance attestations for regulator queries (EZ Stack domain)
//! - **Tax Events** — tax collection pipeline events and withholding records (EZ Stack domain)
...
//! That data lives in the Mass APIs and is accessed via `mez-mass-client`.
...
// -- EZ-Stack-Owned Record Types ---------------------------------------------
//
// These types represent data that genuinely belongs to the EZ Stack,
// NOT Mass primitive data. Mass primitives are accessed via mez-mass-client.

/// Corridor record (API-layer representation).
///
/// Uses [`DynCorridorState`] from `mez-state` for the corridor state, ensuring
...
///
/// Wraps a [`mez_agentic::TaxEvent`] with withholding results and pipeline
/// status for API-layer persistence. Tax events are EZ-Stack-owned data —
...
///
/// Contains EZ-Stack-owned stores (corridors, smart assets, attestations),
...
pub struct AppState {
    ...
    pub mass_client: Option<mez_mass_client::MassClient>,
...
impl AppState {
    ...
    pub fn with_config(
        config: AppConfig,
        mass_client: Option<mez_mass_client::MassClient>,
    ) -> Self {
...
mod tests {
    ...
    fn store_insert_and_get_roundtrip() {
        ...
        assert_eq!(retrieved.jurisdiction_a, "PK-PEZ");
    }
