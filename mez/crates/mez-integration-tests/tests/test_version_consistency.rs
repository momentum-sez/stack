...
/// Locate the repository root (parent of the mez/ workspace directory).
fn repo_root() -> PathBuf {
    let manifest_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    // mez/crates/mez-integration-tests -> mez/ -> stack/
    manifest_dir
        .parent()
        .and_then(|p| p.parent())
        .and_then(|p| p.parent())
...
fn rust_workspace_version_matches_canonical() {
    let cargo_toml = repo_root().join("mez/Cargo.toml");
...
fn openapi_version_matches_canonical() {
    let openapi = repo_root().join("mez/crates/mez-api/src/openapi.rs");
    let content = std::fs::read_to_string(&openapi)
        .unwrap_or_else(|e| panic!("Failed to read {}: {e}", openapi.display()));
    assert!(
        content.contains(&format!("version = \"{CANONICAL_VERSION}\"")),
        "OpenAPI spec in mez-api must contain version = \"{CANONICAL_VERSION}\""
    );
}

#[test]
fn cli_version_matches_canonical() {
    let main_rs = repo_root().join("mez/crates/mez-cli/src/main.rs");
...
fn all_child_crates_inherit_workspace_version() {
    let crates_dir = repo_root().join("mez/crates");
