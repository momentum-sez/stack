...
module.exports = function build_chapter52() {
  return [
    ...
    p("The mez CLI transforms zone configuration into running infrastructure through a five-step pipeline. Each step is idempotent and can be re-run safely."),
    ...codeBlock(
`# 1. Initialize a new zone with jurisdiction profile
mez init --zone my-zone --profile digital-financial-center --jurisdiction AE-ADGM

# 2. Import jurisdiction packs (lawpack, regpack, licensepack)
mez pack import --jurisdiction AE-ADGM --all

# 3. Deploy infrastructure (Docker or Kubernetes)
mez deploy --target docker    # Local / single-server
mez deploy --target k8s       # Kubernetes cluster

# 4. Verify deployment health and compliance readiness
mez verify --zone my-zone --checks health,compliance,connectivity

# 5. Activate corridors for cross-border operations
mez corridor activate --from AE-ADGM --to PK-PEZ --corridor-type bilateral`
    ),
    ...
    table(
      ...
      [
        ["1. Init", "mez init", "< 5s", "zone.yaml, stack.lock", "Delete zone directory"],
        ["2. Pack Import", "mez pack import", "10–30s", "Lawpack, regpack, licensepack in CAS", "mez pack remove"],
        ["3. Deploy", "mez deploy", "2–18 min", "Running services, health endpoints", "mez deploy --destroy"],
        ["4. Verify", "mez verify", "30–60s", "Verification report (JSON)", "No state change"],
        ["5. Corridor", "mez corridor activate", "< 10s", "Corridor state in DRAFT", "mez corridor deactivate"],
      ],
    ...
    p("Running mez deploy --target aws --region me-south-1 executes the following pipeline: validate AWS credentials and check resource quotas, apply Terraform with auto-approve, wait for EKS node readiness, deploy MEZ workloads via Helm, run the verification suite, and output the API endpoint URL. Rollback on failure is automatic: the script captures Terraform state before apply and reverts on any provisioning error."),
    table(
      ...
      [
        ...
        ["3. Deploy", "Helm install: mez-api, mez-worker, monitoring stack", "~2 min", "Helm rollback"],
