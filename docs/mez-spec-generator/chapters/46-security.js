  ...
  return [
    ...
    table(
      ...
      [
        ...
        ["Mass Bridge", "Mass API integration", "Type-safe requests, response validation", "mez-mass-client with typed HTTP methods"],
        ["Cryptographic", "Key material, signatures", "Key isolation, zeroization, constant-time ops", "mez-crypto crate, Zeroize trait, subtle crate"],
        ["Corridor", "Cross-border operations", "Bilateral verification, receipt integrity", "MMR proofs, watcher attestations, threshold sigs"],
        ["Compliance", "Regulatory enforcement", "Tensor evaluation, pack verification", "mez-tensor + mez-pack composition"],
    ...
    table(
      ...
      [
        ...
        ["C4", "Signature verification", "~50,000\u2013200,000", "Verifies an Ed25519 signature inside the circuit, binding the proof to an authorized signer. This is the most expensive per-operation constraint group due to elliptic curve arithmetic in R1CS."],
    ...
    p_runs([bold("Total Circuit Size."), " The complete \u03C0priv circuit comprises approximately 78,000\u2013228,000 R1CS constraints (depending on signature scheme optimization). At current proving speeds (Groth16 on commodity hardware), proof generation takes 2-4 seconds and verification takes under 10 milliseconds. The proof size is constant at 192 bytes (two G1 points and one G2 point in BN254), making it practical for on-chain verification and credential embedding."]),
    p_runs([bold("Constraint Optimization."), " C3 (Merkle membership) and C4 (signature verification) together account for over 40% of the circuit. Two optimization paths are under evaluation: (1) replacing SHA-256 in the Merkle tree with a SNARK-friendly hash (Poseidon), which would reduce C3 to approximately 3,000 constraints, and (2) using Schnorr signatures instead of Ed25519 inside circuits, reducing C4 to approximately 2,500 constraints. These optimizations are tracked in the mez-zkp crate roadmap."]),
    p_runs([bold("Security Level."), " The circuit targets 128-bit security. The BN254 curve provides approximately 128 bits of security against discrete log attacks. For sovereign deployments requiring higher security margins, BLS12-381 (approximately 128 bits) is supported as an alternative curve, at the cost of approximately 20% larger constraints due to the wider field."]),
  ];
};
