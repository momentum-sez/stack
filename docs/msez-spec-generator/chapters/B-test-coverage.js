...
module.exports = function build_appendixB() {
  return [
    ...
    p(
      "The SEZ Stack employs a multi-tier testing methodology with over 3,800 tests executed " +
    ...
    p(
      "The msez-integration-tests crate (107 test files) exercises cross-crate interactions " +
    ...
    p(
      "Contract tests in msez-mass-client validate serialization against the live Mass API " +
      ...
      "codes map correctly to the SEZ Stack error hierarchy. The NADRA adapter includes " +
...
mod tests {
    ...
    table(
      ...
      [
        ["msez-core", "~350", "MCF canonicalization, digest computation, domain enumeration, serde fidelity, sovereignty enforcement"],
        ["msez-crypto", "~300", "Ed25519 sign/verify, MMR append/inclusion/root, CAS store/resolve, SHA-256 centralization"],
        ["msez-vc", "~200", "VC issuance, Ed25519 proof validation, credential registry, asset_id binding"],
        ["msez-state", "~400", "Corridor FSM (exhaustive transition matrix), entity lifecycle, license lifecycle, migration saga, watcher bonds/slashing"],
        ["msez-tensor", "~250", "20-domain evaluation, manifold path optimization, tensor commitments, fail-closed empty slices, aggregate state"],
        ["msez-zkp", "~200", "Mock proof system, Groth16/Plonk backends, production policy enforcement, circuit module stubs"],
        ["msez-pack", "~500", "Lawpack parsing, regpack validation, licensepack submodules, pack composition, Akoma Ntoso integration"],
        ["msez-corridor", "~450", "Dual-commitment receipt chain, compute_next_root, fork resolution with signed attestations, netting, SWIFT adapter, payment rails"],
        ["msez-agentic", "~250", "Trigger taxonomy, policy evaluation, tax pipeline, audit trail, scheduling"],
        ["msez-arbitration", "~200", "Dispute lifecycle, evidence management, escrow, enforcement"],
        ["msez-mass-client", "~200", "Contract tests for 5 Mass APIs, NADRA adapter, retry logic, error mapping"],
        ["msez-api", "~400", "Route handlers, middleware, orchestration, database operations, auth"],
        ["msez-cli", "~150", "Corridor CLI, artifact operations, lockfile generation, signing"],
        ["msez-schema", "~100", "Schema validation (Draft 2020-12), $ref resolution, codegen policy"],
        ["msez-compliance", "~50", "Evaluator composition, jurisdiction-aware evaluation"],
        ["msez-integration-tests", "~350", "Cross-crate workflows, serde round-trips, API contract tests, determinism"],
      ],
      [2400, 1200, 5760]
    ),
  ];
