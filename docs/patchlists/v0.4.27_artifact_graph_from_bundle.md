...
### `msez artifact graph verify --from-bundle <zip>`
...
This validates the `MSEZArtifactGraphVerifyReport` format produced by `msez artifact graph verify` and embedded as `manifest.json` in witness bundles.
