...
use mez_core::{CanonicalBytes, ComplianceDomain, ContentDigest, JurisdictionId, WatcherId};
...
use mez_crypto::{sha256_digest, MerkleMountainRange, SigningKey};
...
use mez_zkp::mock::{MockCircuit, MockProofSystem, MockProvingKey, MockVerifyingKey};
use mez_zkp::traits::ProofSystem;
use serde_json::json;
use std::panic;
...
fn entity_id_nil_uuid() {
    ...
    let result: Result<mez_core::EntityId, _> = serde_json::from_str(&format!("\"{}\"", nil_uuid));
    assert!(
        result.is_ok(),
...
fn vc_signing_deterministic() {
    ...
    let make_vc = || {
        ...
        vc.sign_ed25519(
            ...
            Some(mez_core::Timestamp::from_rfc3339("2026-01-15T12:00:00Z").unwrap()),
        )
        .unwrap();
...
fn swift_xml_deterministic_100_runs() {
    let swift = SwiftPacs008::new("MEZSEXX");
    let instruction = SettlementInstruction {
...
use mez_state::{SlashingCondition, Watcher};

#[test]
fn watcher_slash_equivocation_100_percent() {
    let mut watcher = Watcher::new(mez_core::WatcherId::new());
...
use mez_state::MigrationBuilder;

#[test]
fn migration_saga_expired_deadline_forces_timeout() {
    let past = chrono::Utc::now() - chrono::Duration::hours(1);
    let mut saga = MigrationBuilder::new(mez_core::MigrationId::new())
    ...
    match result {
        Ok(state) => {
            assert_eq!(
                state,
                mez_state::MigrationState::TimedOut,
...
fn cas_store_deterministic_digest_100_runs() {
    ...
    for _ in 0..100 {
        ...
        let cas = mez_crypto::ContentAddressedStore::new(tmp.path());
...
fn pack_validate_compliance_domain_all_known_domains() {
    ...
    // from ComplianceDomain::as_str() in mez-core/src/domain.rs.
    let domains = [
        "aml",
        "kyc",
        "sanctions",
...
use mez_agentic::{AuditEntry, AuditEntryType, AuditTrail, PolicyEngine, Trigger, TriggerType};
...
fn determinism_swift_pacs008_same_inputs() {
    let swift1 = SwiftPacs008::new("MEZTEST");
    let swift2 = SwiftPacs008::new("MEZTEST");
    let instruction = SettlementInstruction {
...
fn licensepack_is_expired_string_date_comparison() {
    ...
    let _jid = JurisdictionId::new("PK-PEZ").unwrap();
    let license = License {
        ...
        holder_did: Some("did:mez:test".to_string()),
...
fn licensepack_holder_did_lookup_empty_string() {
    ...
    let jid = JurisdictionId::new("PK-PEZ").unwrap();
...
fn licensepack_resolve_refs_missing_fields() {
    ...
    let refs = mez_pack::licensepack::resolve_licensepack_refs(&zone).unwrap();
