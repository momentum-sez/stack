...
module.exports = function build_chapter51() {
  return [
    ...
    p("The Terraform configuration provisions a complete AWS environment for production MEZ deployments. All resources are tagged with environment, project, and cost-center labels for operational tracking."),
    table(
...
module "vpc" {
  ...
  enable_dns_hostnames = true
    ...
    p("An EKS cluster (Kubernetes 1.29) runs MEZ workloads with managed node groups. Terraform provisions the cluster, node groups, and all Kubernetes resources via the Kubernetes and Helm providers."),
    table(
      ["Resource", "Type", "Configuration", "Purpose"],
      [
        ["mez-api", "Deployment", "Rolling update (maxSurge: 1, maxUnavailable: 0)", "Primary API server"],
        ["mez-worker", "Deployment", "Rolling update, no external ports", "Background task processor"],
    ...
    table(
      ...
      [
        ["standard", "m6i.xlarge", "2 / 4", "100 GB gp3", "workload=mez, tier=standard"],
        ["enterprise", "m6i.2xlarge", "3 / 8", "200 GB gp3", "workload=mez, tier=enterprise"],
        ["sovereign-govos", "m6i.4xlarge", "4 / 16", "500 GB io2", "workload=mez, tier=sovereign"],
