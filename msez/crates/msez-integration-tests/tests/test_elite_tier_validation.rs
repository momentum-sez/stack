...
//! multiple tiers of the SEZ stack: canonicalization correctness, corridor
//! state alignment with the spec, compliance domain count, digest format,
//! and canonical bytes UTF-8 validity.

use msez_core::{sha256_digest, CanonicalBytes, CanonicalizationError, ComplianceDomain};
...
fn digest_format_correct() {
    ...
    // Algorithm tag
    assert_eq!(digest.algorithm(), msez_core::DigestAlgorithm::Sha256);
}
