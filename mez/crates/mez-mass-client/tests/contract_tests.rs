...
//!
//! These tests validate that the Rust client types in `mez-mass-client` match
...
//!
//! ```bash
//! cargo test -p mez-mass-client contract_test
//! ```
//!
//! ## Staleness checks (fetch live specs and compare)
//!
//! ```bash
//! cargo test -p mez-mass-client contract_staleness -- --ignored
//! ```
...
fn contract_test_organization_deserializes_from_spec_shape() {
    ...
    let entity: mez_mass_client::entities::MassEntity = serde_json::from_value(json)
...
fn contract_test_organization_entity_status_enum_variants() {
    ...
    for status in &known_statuses {
        ...
        let result: Result<mez_mass_client::entities::MassEntity, _> =
    ...
    let entity: mez_mass_client::entities::MassEntity =
        serde_json::from_value(json).expect("MassEntity should handle unknown status");
    assert_eq!(
        entity.status,
        Some(mez_mass_client::entities::MassEntityStatus::Unknown)
    );
}
...
fn contract_test_organization_search_response_deserializes() {
    ...
    let result: mez_mass_client::entities::SearchOrganizationsResponse =
        serde_json::from_value(json)
...
fn contract_test_treasury_deserializes_from_spec_shape() {
    ...
    let treasury: mez_mass_client::fiscal::MassTreasury = serde_json::from_value(json)
        .expect("MassTreasury must deserialize from Treasury-shaped JSON");
    assert_eq!(treasury.entity_id, "org-123");
    assert_eq!(
        treasury.context,
        Some(mez_mass_client::fiscal::MassTreasuryContext::UnitFinance)
    );
}
...
fn contract_test_bank_account_deserializes_from_spec_shape() {
    ...
    let account: mez_mass_client::fiscal::MassFiscalAccount = serde_json::from_value(json)
...
fn contract_test_payment_deserializes_from_spec_shape() {
    ...
    let payment: mez_mass_client::fiscal::MassPayment = serde_json::from_value(json)
...
fn contract_test_treasury_context_enum_variants() {
    ...
    for v in &known {
        ...
        let result: Result<mez_mass_client::fiscal::MassTreasury, _> =
            serde_json::from_value(json);
        assert!(
            result.is_ok(),
...
fn contract_test_payment_status_enum_variants() {
    ...
    for v in &known {
        ...
        let result: Result<mez_mass_client::fiscal::MassPayment, _> =
            serde_json::from_value(json);
        assert!(
            result.is_ok(),
...
fn contract_test_consent_deserializes_from_spec_shape() {
    ...
    let consent: mez_mass_client::consent::MassConsent = serde_json::from_value(json)
        .expect("MassConsent must deserialize from Consent-shaped JSON");
    assert_eq!(consent.organization_id, "org-123");
    assert_eq!(
        consent.status,
        Some(mez_mass_client::consent::MassConsentStatus::Pending)
    );
}
...
fn contract_test_consent_status_enum_variants() {
    ...
    for v in &known {
        ...
        let result: Result<mez_mass_client::consent::MassConsent, _> =
            serde_json::from_value(json);
        assert!(
            result.is_ok(),
...
fn contract_test_cap_table_deserializes_from_spec_shape() {
    ...
    let cap_table: mez_mass_client::ownership::MassCapTable = serde_json::from_value(json)
...
fn contract_test_submission_response_deserializes_from_spec_shape() {
    ...
    let submission: mez_mass_client::templating::SubmissionResponse =
        serde_json::from_value(json)
...
fn contract_test_template_deserializes_from_spec_shape() {
    ...
    let template: mez_mass_client::templating::Template = serde_json::from_value(json)
...
fn contract_test_signing_order_enum_variants() {
    ...
    for v in &known {
        let result: Result<mez_mass_client::templating::SigningOrder, _> =
...
fn contract_test_signing_role_enum_variants() {
    ...
    for v in &known {
        let result: Result<mez_mass_client::templating::SigningRole, _> =
            serde_json::from_value(serde_json::json!(v));
...
#[tokio::test]
#[ignore] // Run with: cargo test -p mez-mass-client contract_staleness -- --ignored
    ...
    if !added.is_empty() || !removed.is_empty() || !field_diffs.is_empty() {
        let mut msg = format!(
            "{spec_name} spec has drifted. \
             Run `./mez/crates/mez-mass-client/scripts/refresh-specs.sh` and review changes.\n"
        );
        if !added.is_empty() {
