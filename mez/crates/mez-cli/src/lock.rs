...
//! ## Spec Reference
//!
//! Matches the behavior of `tools/mez.py lock` which produces canonical JSON
...
/// Arguments for the `mez lock` subcommand.
#[derive(Args, Debug)]
pub struct LockArgs {
...
///
/// Delegates to [`mez_core::sha256_raw`] — the sole raw-byte SHA-256
/// implementation per CLAUDE.md §V.5.
fn sha256_of_bytes(bytes: &[u8]) -> String {
    mez_core::sha256_raw(bytes)
}
...
mod tests {
    ...
    fn sha256_file_nonexistent_returns_error() {
        let result = sha256_file(Path::new("/tmp/mez-no-such-file-abc123.txt"));
    ...
    fn walkdir_recursive_nonexistent_dir() {
        let results = walkdir_recursive(Path::new("/tmp/mez-no-such-dir-xyz123"));
    ...
    fn find_profile_missing_dir_errors() {
        let result = find_profile(
            Path::new("/tmp/mez-no-profiles-dir-xyz123"),
            "test_profile",
        );
        assert!(result.is_err());
        let err_msg = result.unwrap_err().to_string();
