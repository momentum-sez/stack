...
### New: CLI `msez asset module init <asset_id>`
...
All `msez asset state *` commands now accept an **optional positional** `<path>` (asset module directory or `asset.yaml`). When provided:
...
msez asset state receipt-init modules/smart-assets/<asset_id> --sequence 0 --prev-root genesis --sign --key jwk.json
