...
use mez_corridor::swift::{SettlementInstruction, SettlementRailError};
use mez_corridor::{SettlementRail, SwiftPacs008};

fn sample_instruction() -> SettlementInstruction {
    SettlementInstruction {
        message_id: "MEZ-2026-IT-001".to_string(),
        debtor_bic: "HABORAEK".to_string(),
        debtor_account: "PK36HABB0000001123456702".to_string(),
        debtor_name: "Momentum EZ Operator PKR".to_string(),
...
fn swift_instruction_creation() {
    let adapter = SwiftPacs008::new("MEZSEXX");
    ...
    assert!(xml.contains("<MsgId>MEZ-2026-IT-001</MsgId>"));
    assert!(xml.contains("Momentum EZ Operator PKR"));
    assert!(xml.contains("Momentum EZ Operator AED"));
}
...
fn swift_default_remittance() {
    ...
    assert!(xml.contains("EZ Settlement"));
}
