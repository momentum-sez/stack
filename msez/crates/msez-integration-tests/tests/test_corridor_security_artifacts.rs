...
use msez_core::{sha256_digest, CanonicalBytes, CorridorId};
use msez_crypto::{ContentAddressedStore, SigningKey, VerifyingKey};
use rand_core::OsRng;
use serde_json::json;
...
fn security_artifact_cas_storage() {
    ...
    let recovered_sig = msez_crypto::Ed25519Signature::from_hex(stored_sig_hex).unwrap();
