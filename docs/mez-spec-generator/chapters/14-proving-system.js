...
module.exports = function build_chapter14() {
  return [
    ...
    p("The EZ Stack\u2019s proving system is implemented in the mez-zkp crate, which defines a sealed ProofSystem trait with pluggable backends. The architecture separates proof system selection from business logic: any component that requires a ZK proof depends on the trait, not a concrete backend. This chapter specifies the implemented backends, the production policy enforcement mechanism, and the planned Mass L1 proving pipeline."),

    // --- 14.1 ProofSystem Trait ---
    h2("14.1 ProofSystem Trait"),
    p("The sealed ProofSystem trait defines the interface that all proof backends must implement. It is sealed (not implementable outside mez-zkp) to prevent untrusted third-party backends from being injected into the verification pipeline."),

    ...codeBlock(
      "/// Sealed trait for zero-knowledge proof backends (mez-zkp)\n" +
    ...
    table(
      ...
      [
        ["Mock (SHA-256 deterministic)", "mez-zkp::mock", "Testing only", "None (not cryptographic)", "Development, CI, property testing. Proofs are SHA-256 hashes of inputs \u2014 deterministic but not zero-knowledge"],
        ["Groth16", "mez-zkp::groth16", "Implemented", "Trusted setup (per-circuit SRS)", "Compact proofs for on-chain verification. Constant proof size (288 bytes). Pairing-based verification"],
        ["Plonk", "mez-zkp::plonk", "Implemented", "Universal setup (updateable SRS)", "General-purpose ZK proofs. Larger proofs than Groth16 but no per-circuit trusted setup"],
      ],
    ...
    p("The mez-zkp::policy module prevents the catastrophic scenario where mock proofs are accepted in a production deployment. The policy is configured via a content-addressed policy artifact that specifies which backends are permitted for each deployment environment."),

    p_runs([
      bold("Compile-time enforcement. "),
      "CI gates verify that release builds do not enable the mock feature flag. cargo metadata checks confirm that mez-zkp is compiled without mock features in release profiles."
    ]),
    p_runs([
    ...
    p("The mez-zkp crate defines five circuit modules covering the governance operations that require zero-knowledge proofs. These modules define the circuit interfaces and witness structures; the concrete constraint systems will be implemented when the ZK layer activates in Phase 4."),
    ...
    p("The Mass Protocol\u2019s L1 settlement layer specifies a multi-layer proof aggregation pipeline that progressively reduces verification cost. This pipeline is not yet implemented in the EZ Stack; it describes the target architecture that will be integrated when the Mass L1 is deployed."),
