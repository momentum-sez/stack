# Integration Stress Test Report — SEZ Stack
...
The SEZ Stack compiles cleanly, passes 3,516 tests with 0 failures, and `cargo clippy --workspace -- -D warnings` is clean. The individual crates — tensor, pack, corridor, state FSM, crypto, VC — are well-implemented in isolation. **The problem is not in the parts. The problem is in the wiring.**

The modules were built in parallel, not in serial. Each crate works when tested alone, but the composition layer (`msez-api`) that should orchestrate them into end-to-end flows has gaps that would cause real failures in sovereign deployment. This report identifies those gaps by tracing actual request paths through the system.
...
Request → mass_proxy::create_entity()
  ...
  → msez_mass_client::entities::create()  [real HTTP to Mass org-info API]
  → orchestration::orchestrate_entity_creation()
    → issue_compliance_vc()  [Ed25519 signed VC via msez-vc]
...
Request → mass_proxy::initiate_payment()
  ...
  → msez_mass_client::fiscal::create_payment()  [real HTTP to Mass treasury API]
...
**Location**: Every write handler in `msez-api/src/routes/`
...
**Location**: `msez-api/src/routes/agentic.rs`
...
**Fix**: Either use `msez_mass_client` types directly as API request types, or add compile-time mapping validation via `From` trait implementations with exhaustive field coverage.

---

### HOLE 4: No Contract Tests Against Live Mass API Specs (P0-008)

**Location**: `msez-mass-client/tests/`

**The Problem**: The Mass client has wiremock tests (good for regression) but no tests that validate against the actual Swagger specs served by the live Mass API endpoints. The types in `msez-mass-client` were written by reading the API docs — they may have drifted from the live schemas.
...
**Location**: `msez-corridor/src/swift.rs`, `msez-api/src/routes/settlement.rs`
...
**Location**: `msez-pack/src/composition.rs` (implemented), `msez-api/src/routes/` (no endpoint)
...
**Location**: `msez-mass-client/src/identity.rs`
...
**Location**: `msez-mass-client/src/` (all sub-clients)
...
**Location**: `msez-api/src/state.rs::SmartAssetRecord`
...
**Location**: `msez-api/src/routes/mass_proxy.rs::create_account()`
...
The AUDIT_FINDINGS.md marks P1-004 as RESOLVED, and the code confirms that write endpoints DO compose compliance evaluation + Mass API + VC issuance. However, **read endpoints remain pure proxies** — `GET /v1/entities/:id` fetches from Mass without any SEZ Stack enrichment. This means:
...
### Tension 3: msez-compliance Crate vs. msez-api/compliance Module

Two different compliance interfaces exist:

1. `msez-compliance` crate: `build_tensor(jurisdiction_id, applicable_domains, sanctions_entries)` — takes regpack data as input, returns `ComplianceTensor<RegpackJurisdiction>`
2. `msez-api/src/compliance.rs`: `build_tensor(jurisdiction_id)` — takes only a string, returns `ComplianceTensor<DefaultJurisdiction>`

The API uses its own simpler version (#2) and ignores the richer crate (#1). The `msez-compliance` crate — designed to bridge packs and tensor — is not wired into the HTTP layer. The `SanctionsEvaluator` from `msez-compliance::evaluators` is never used in request handling.

**Resolution**: The API's `build_tensor()` should use `msez-compliance::build_tensor()` with actual regpack data from the zone bootstrap.
...
**Resolution**: Wire `ZoneContext::sanctions_checker` → `msez-compliance::SanctionsEvaluator` → `ComplianceTensor` evaluation pipeline. Currently, the sanctions hard-block check looks at the tensor's `Sanctions` domain state, which defaults to `Pending` (never `NonCompliant`), meaning **sanctions screening never actually blocks**.
...
| 3 | Contract tests against live Mass API Swagger specs | Type drift between client and API | msez-mass-client/tests/ |
...
| 6 | Verify identity endpoint paths exist on live Mass APIs | Identity verification will 404 | msez-mass-client/identity.rs |
| 7 | Use msez-compliance crate instead of local build_tensor | Richer compliance evaluation unused | msez-api/src/compliance.rs |
| 8 | Add 5xx retry logic to Mass client | Transient outages cascade | msez-mass-client/src/*.rs |
...
| 12 | Complete OwnershipClient (share transfers, investors) | Cap table mutations blocked | msez-mass-client/ownership.rs |
