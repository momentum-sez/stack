...
pub struct SwiftPacs008 {
    /// Instructing agent BIC (the EZ settlement node).
...
impl SwiftPacs008 {
    ...
    ///
    /// `instructing_agent_bic` is the BIC of the EZ settlement node
...
mod tests {
    ...
    fn sample_instruction() -> SettlementInstruction {
        SettlementInstruction {
            message_id: "MEZ-2026-001".to_string(),
    ...
    fn generates_valid_xml() {
        ...
        assert!(xml.contains("<MsgId>MEZ-2026-001</MsgId>"));
    ...
    fn accepts_11_char_bic() {
        let adapter = SwiftPacs008::new("MEZSEXXABC");
        let mut instr = sample_instruction();
