...
module.exports = function build_chapter07() {
  return [
    ...
    p("Corridor capabilities focus on trade corridors with receipt chain tracking for goods movement. Each corridor maintains a cryptographically verifiable record of every document, inspection, and handoff in the supply chain. The compliance tensor is configured with elevated weights for CUSTOMS, TRADE, SANCTIONS, and AML_CFT domains, reflecting the regulatory priorities of trade facilitation zones."),
    ...
    p("Asset-history-bundle deployments activate the receipt chain subsystem at maximum fidelity. Every state transition of a tracked asset generates a receipt: creation, inspection, certification, transfer, encumbrance, release, and disposal. Receipts are cryptographically chained using the MMR (Merkle Mountain Range) data structure, enabling efficient proof of any historical state. The credential module issues Verifiable Credentials for certifications, appraisals, and compliance attestations that can be independently verified without contacting the issuing system."),
